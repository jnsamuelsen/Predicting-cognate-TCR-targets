...
plt.bar(file_list, peptide_scores, color = 'maroon')
plt.xlabel('PDB SW structure', fontsize = 15)
plt.ylabel('Score', fontsize = 15)
plt.xticks(rotation=90, fontsize = 8)
plt.title('Leave one out swapped peptide scorings', fontsize = 20)
fig.savefig("SW_scorings.png")

peptide_scores_df = pd.DataFrame(peptide_scores, index = file_list).astype(float)

peptide_scores_df.to_csv('peptide_scores_SW.csv')