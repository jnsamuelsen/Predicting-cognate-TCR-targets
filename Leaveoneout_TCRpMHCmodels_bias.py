...
path_of_dir_scoring = '/mnt/c/Users/jeppe/Desktop/Speciale2022/Scripts/Scoringstructures/PDB/WT_weighted'
ext = '_cleaned.pdb'
...
for leaveoutfile in os.listdir(path_of_dir_scoring):
    ...
    # Leave out TCR-p structure
    leave_out_ab = find_tcr_p_interactions(WTfile)
    ...
    for files in os.listdir(path_of_dir):
        ...
        ## Proceed
        if len(alignment_df.at[leaveoutfile[:4], 'peptide_seq']) != len(alignment_df.at[files[:4], 'peptide_seq']):
        ...
        elif distance.levenshtein(alignment_df.at[leaveoutfile[:4], 'peptide_seq'], alignment_df.at[files[:4], 'peptide_seq']) >= 3:
            if files.endswith(ext):
...
plt.bar(file_list, peptide_scores, color = 'royalblue')
plt.xlabel('PDB WT structure', fontsize = 15)
plt.ylabel('Score', fontsize = 15)
plt.xticks(rotation=90, fontsize = 8)
plt.title('Leave one out weighted wildtype peptide scorings', fontsize = 20)
fig.savefig("weighted_WT_scorings_minusbias.png")

peptide_scores_df = pd.DataFrame(peptide_scores, index = file_list).astype(float)

peptide_scores_df.to_csv('peptide_scores_weighted_WT_minusbias.csv')